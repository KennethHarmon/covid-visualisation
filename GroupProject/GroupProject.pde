...
void setupProgram() {
  ...
  histogram = new HistogramModule(width/2 + MODULE_PADDING/2, 2 * MODULE_PADDING + (height - 4 * MODULE_PADDING) / 8, (width - 3 * MODULE_PADDING) / 2, (height - 4 * MODULE_PADDING) * 4/8, FilterData.createTotalCasesPerTime(stateAdminAreaCases30, myCompleteDataList), 5); 
