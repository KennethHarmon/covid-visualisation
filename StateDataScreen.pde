...
RadioButtonsModule radioButtons;
  ...
  StateDataScreen(String stateName) {
    ...
    newCases2 = new NewCasesModule(width - MODULE_PADDING - (width - 4 * MODULE_PADDING) / 3, MODULE_PADDING, (width - 4 * MODULE_PADDING) / 3, (height - 4 * MODULE_PADDING) / 8, FilterData.findNewCasesForCounty(allStateEntriees, stateName,0));
    super.addModules(
      ...
      new HistogramModule(MODULE_PADDING, 3 * MODULE_PADDING + 2 * (height - 4 * MODULE_PADDING) / 8, width - 2 * MODULE_PADDING, (height - 4 * MODULE_PADDING) * 6 / 8, FilterData.filterHashMapByDate(FilterData.createStateCasesPerTime(stateName, stateCaseNumbers, myCompleteDataList), new Date(120, 0, 0)), 5), 
      newCases2,
      radioButtons
      );
  }
